...
class TokenType(Enum):
    ...
    LITERAL_START = auto()
    LITERAL_END = auto()
    TRIPLE_LITERAL_START = auto()
    TRIPLE_LITERAL_END = auto()
    ASTERISK = auto()
...
class AtacamaLexer:
    ...
    def collect_url(self) -> str:
        """Collect a valid URL using regular expressions."""
        ...
        if match:
            url = match.group(0)
            # Move the lexer position forward by the length of the URL
            self.pos += len(url) - 1  # -1 because self.advance will increment by 1
    ...
    def get_next_token(self) -> Optional[Token]:
        ...
        while self.current_char is not None:
            ...
            # Handle URLs
            if self.current_char == 'h':  # Possible start of "http"
            ...
            # Collect contiguous text, including spaces
            text = []
            while self.current_char and not (
                self.current_char in {'<', '[', '(', ')', ']', '*', '-', '\n'}
                or '\u4e00' <= self.current_char <= '\u9fff'
                or (self.current_char == 'h' and self.peek(1) == 't' and self.peek(2) == 't' and self.peek(3) == 'p')  # URL start
            ):
