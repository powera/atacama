...
class SymbolTableVisitor(ast.NodeVisitor):
    ...
    def __init__(self, predefined_names: Optional[Set[str]] = None):
        self.ctx = Context()
        ...
        if predefined_names:
            self.ctx.defined_names.update(predefined_names)
...
def get_local_modules(root_dir: Union[str, Path]) -> Set[str]:
...
def get_module_level_definitions(tree: ast.AST) -> Set[str]:
    """Collect module-level function/class names for forward references."""
    names: Set[str] = set()
...
def check_imports(file_path: Path, local_modules: Set[str]) -> List[str]:
    ...
    try:
        ...
        is_test_file = "tests" in file_path.parts or any(
            part.startswith("test_") for part in file_path.parts
        )
        ...
        for node in ast.walk(tree):
            if isinstance(node, (ast.Import, ast.ImportFrom)):
                ...
                if isinstance(node, ast.Import):
                    for import_alias in node.names:
                        base_package = import_alias.name.split(".")[0]
                        if base_package in stdlib_modules:
                            continue
                        if (
                            base_package in local_modules
                            or base_package.startswith(
                                (
                                    "common",
                                    "atacama",
                                    "blog",
                            ...
                            or base_package in ("constants",)
                        ):
                            continue
                        if base_package not in requirements:
                            errors.append(
                                f"Third-party import '{base_package}' not found in requirements.txt (package: {requirements.get(base_package.lower(), base_package)}): {file_path}"
                            )
                elif node.module:
        ...
        module_level_definitions = get_module_level_definitions(tree)
        symbol_visitor = SymbolTableVisitor(predefined_names=module_level_definitions)
        ...
        unused_imports = (
            import_names - symbol_visitor.ctx.used_names - {"_"} - type_checking_imports
        )
        if is_init_file or is_test_file:
            unused_imports = set()
        for unused_name in unused_imports:
            errors.append(f"Unused import '{unused_name}' in {file_path}")
        ...
        for undefined_name in sorted(undefined):
            errors.append(f"Undefined variable '{undefined_name}' used in {file_path}")
