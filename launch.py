...
def init_system(log_level="INFO", app_log_level="DEBUG", service=None):
    ...
    configure_logging(log_level=log_level, app_log_level=app_log_level, log_filename=log_filename)
...
def parse_args():
    ...
    parser.add_argument(
        "--mode",
        choices=["web", "trakaido", "spaceship"],
...
def main():
    ...
    try:
        ...
        if args.web:
            ...
            run_server(
                host=args.host, port=port, debug=args.debug or args.dev, blueprint_set="BLOG"
            )
        elif args.trakaido:
            from atacama.server import run_server
            ...
            run_server(
                host=args.host, port=port, debug=args.debug or args.dev, blueprint_set="TRAKAIDO"
            )
        elif args.spaceship:
