...
def run_react_compiler_tests(
    ...
    try:
        ...
        runner = unittest.TextTestRunner(
            verbosity=verbosity, failfast=fail_fast, buffer=not verbose
        )
        ...
        try:
            ...
            for attr_name in dir(module):
                ...
                if (
                    ...
                    test_methods = [method for method in dir(attr) if method.startswith("test_")]
                    if test_methods:
...
Note: These tests require Node.js and npm to be installed.
    ...
    parser.add_argument("--pattern", help='Only run tests matching pattern (e.g., "*basketball*")')
    ...
    parser.add_argument(
        "--list-tests", action="store_true", help="List available tests without running them"
    )
