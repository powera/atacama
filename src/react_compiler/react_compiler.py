...
class WidgetBuilder:
    ...
    DEFAULT_EXTERNAL_PACKAGES = {"react": "React", "react-dom": "ReactDOM"}
    ...
    def _transform_hook_for_bundling(
        self, hook_code: str, hook_name: str, is_first_hook: bool = True
    ) -> str:
        """
        ...
        code = re.sub(
            r"export\s+const\s+" + re.escape(hook_name) + r"\s*=", f"const {hook_name} =", code
        )
    ...
    def _prepare_widget_code_with_hooks(self, widget_code: str, hooks_needed: List[str]) -> str:
        ...
        if hooks_needed:
            ...
            if react_match:
                ...
                widget_code = re.sub(
                    react_import_pattern, "import React from 'react';\n", widget_code
                )
                logger.info(
                    "Replaced React destructured import with default import to avoid duplicates"
                )
            else:
                ...
                destructured_only_pattern = (
                    r'import\s+\{\s*([^}]+)\s*\}\s+from\s+[\'"]react[\'"];?\n?'
                )
                if re.search(destructured_only_pattern, widget_code):
                    # Replace with React default import
                    widget_code = re.sub(
                        destructured_only_pattern, "import React from 'react';\n", widget_code
                    )
                    logger.info(
                        "Replaced React destructured-only import with default import to avoid duplicates"
                    )
    ...
    def _handle_exports(self, widget_code: str, widget_name: str) -> str:
        ...
        for pattern, pattern_type in export_patterns:
            ...
            if match:
                ...
                logger.info(
                    f"Found existing export: {exported_name or 'anonymous'} (type: {export_type})"
                )
                break
        ...
        if has_export:
            if exported_name and exported_name != widget_name:
                ...
                logger.info(
                    f"Replacing exported component name '{exported_name}' with '{widget_name}'"
                )
                ...
                if export_type == "function":
                    ...
                    code = re.sub(
                        rf"export\s+default\s+function\s+{re.escape(exported_name)}\s*\(",
                        f"export default function {widget_name}(",
                        code,
                    )
                else:
                    ...
                    component_def_patterns = [
                        (
                            rf"function\s+{re.escape(exported_name)}\s*\(",
                            f"function {widget_name}(",
                        ),
        ...
        try:
            ...
            for dep in external_dependencies:
                if dep in self.EXTERNAL_PACKAGE_MAPPINGS:
                    ...
                    logger.warning(
                        f"Using default external name '{externals[dep]}' for dependency '{dep}'"
                    )
...
"""
    ...
    def check_react_libraries(self, code):
        ...
        target_libraries = ["recharts", "lodash", "d3", "axios", "date-fns", "lucide-react"]
        ...
        for imp in imports:
            ...
            if imp.startswith("./") or imp.startswith("../"):
                hook_name = (
                    imp.replace("./", "").replace("../", "").replace(".js", "").replace(".jsx", "")
                )
                if hook_name in self.BUILT_IN_HOOKS:
            ...
            if base_lib in target_libraries and base_lib not in found_libraries:
                ...
                base_lib not in ["react", "react-dom"]
                and not base_lib.startswith(".")
                and base_lib not in other_libraries
            ):
        ...
        if other_libraries:
            warnings.append(
                f"Found {len(other_libraries)} non-target libraries: {', '.join(other_libraries)}"
            )
