...
class AtacamaLexer:
    ...
    def _try_handle_list_marker(self, tok_line: int, tok_col: int) -> Optional[Token]:
        ...
        token_type = None
        if current_marker_char == "*" and (
            next_char_after_marker and next_char_after_marker.isspace()
        ):
            token_type = TokenType.BULLET_LIST_MARKER
        elif current_marker_char == "#" and (
            next_char_after_marker and next_char_after_marker.isspace()
        ):
    ...
    def _should_break_text_parsing(self) -> bool:
        ...
        if self.text.startswith("--MORE--", self.pos):
            return True
        peek4 = self._peek(4)
        if self.text.startswith("----", self.pos) and (
            peek4 is None or peek4.isspace() or peek4 == "\n"
        ):
            return True
    ...
    def get_next_token(self) -> Optional[Token]:
        ...
        token = self._try_handle_delimiters(
            tok_line, tok_col, "<<<", ">>>", TokenType.MLQ_START, TokenType.MLQ_END
        )
        if token:
            return token
        token = self._try_handle_delimiters(
            tok_line, tok_col, "<<", ">>", TokenType.LITERAL_START, TokenType.LITERAL_END
        )
        if token:
            return token
        ...
        token = self._try_handle_delimiters(
            tok_line, tok_col, "[#", "#]", TokenType.TITLE_START, TokenType.TITLE_END
        )
        if token:
            return token
        token = self._try_handle_delimiters(
            tok_line, tok_col, "[[", "]]", TokenType.WIKILINK_START, TokenType.WIKILINK_END
        )
        if token:
            return token
    ...
    def tokenize(self, text: str) -> Generator[Token, None, None]:
        ...
        while self.current_char is not None:
            ...
            if token:  # Ensure token is not empty
                ...
                raise LexerError(
                    "Lexer failed to produce a token or advance.", self.line, self.column
                )
