...
class User(Base):
    ...
    admin_channel_access: Mapped[Optional[Dict]] = mapped_column(
    ...
    auth_tokens: Mapped[List["UserToken"]] = relationship(
...
class Quote(Message):
    ...
    commentary: Mapped[Optional[str]] = mapped_column(
        Text
    )  # For snowclone explanations or personal meanings

    # Relationship with emails that reference this quote
    emails: Mapped[List["Email"]] = relationship(
        "Email", secondary="email_quotes", back_populates="quotes", lazy="selectin"
    )

    __mapper_args__ = {"polymorphic_identity": MessageType.QUOTE}
...
class Email(Message):
    ...
    public_content: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True
    )  # Extracted public version (<<PRIVATE:>> markers stripped)
    public_processed_content: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True
    )  # Rendered HTML for public version
    ...
    chinese_annotations: Mapped[Optional[Dict]] = mapped_column(
        Text
    )  # {position: {hanzi: str, pinyin: str, definition: str}}
    llm_annotations: Mapped[Optional[Dict]] = mapped_column(
        Text
    )  # {position: {type: str, content: str}}
    english_annotations: Mapped[Optional[Dict]] = mapped_column(
        Text
    )  # {word: {guid, lemma, definition, pos_type, ...}}

    # Quote relationships
    quotes: Mapped[List["Quote"]] = relationship(
        "Quote", secondary="email_quotes", back_populates="emails", lazy="selectin"
    )

    __mapper_args__ = {"polymorphic_identity": MessageType.EMAIL}
...
class Article(Message):
    ...
    __mapper_args__ = {"polymorphic_identity": MessageType.ARTICLE}
...
class ReactWidget(Message):
    ...
    versions: Mapped[List["WidgetVersion"]] = relationship(
        ...
        foreign_keys="[WidgetVersion.widget_id]",
        cascade="all, delete-orphan",
    )
    active_version: Mapped[Optional["WidgetVersion"]] = relationship(
        "WidgetVersion", foreign_keys=[active_version_id], post_update=True
    )
    ...
    __mapper_args__ = {"polymorphic_identity": MessageType.WIDGET}
    ...
    def validate_slug(self, key, slug):
        ...
        if not re.match(r"^[a-z0-9]+(?:-[a-z0-9]+)*$", slug.lower()):
            raise ValueError(
                "Slug format is invalid. Must contain only lowercase letters, numbers, and hyphens. Cannot start or end with hyphens, and cannot have consecutive hyphens. Examples: 'my-widget', 'widget123', 'test-app-2'"
            )
        return slug.lower()
...
class WidgetVersion(Base):
    ...
    )  # Whether this version compiles/works
    build_error: Mapped[Optional[str]] = mapped_column(Text)  # Build error if any

    # Relationships
    widget: Mapped["ReactWidget"] = relationship(
        "ReactWidget", back_populates="versions", foreign_keys=[widget_id]
    )
    previous_version: Mapped[Optional["WidgetVersion"]] = relationship(
        "WidgetVersion", remote_side=[id]
    )
