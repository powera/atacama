    ...
    return Schema(
        ...
        "Metadata about a quote including analysis and attribution",
        {
            "theme": SchemaProperty(
                "string", "The main theme or topic (e.g. 'love', 'persistence', 'technology')"
            ),
            "tone": SchemaProperty(
                "string", "The emotional tone (e.g. 'inspirational', 'humorous', 'critical')"
            ),
            "attribution": SchemaProperty(
                "object",
                "Information about the quote's origin",
                properties={
                    "author_type": SchemaProperty(
                        "string", "Type of author", enum=["historical", "fictional"]
                    ),
                    "speaker": SchemaProperty(
                        "string", "The character/person who speaks the quote"
                    ),
                    "context": SchemaProperty(
                        "string", "Publication/work where quote appears, or historical context"
                    ),
                    "time_period": SchemaProperty(
                        "string", "Historical or fictional time period of the quote"
                    ),
                },
            ),
            "interpretation": SchemaProperty(
                "string", "A brief interpretation of the quote's meaning"
            ),
            "keywords": SchemaProperty("array", "Relevant keywords", items={"type": "string"}),
            "related_topics": SchemaProperty(
                "array", "Related topics or themes", items={"type": "string"}
            ),
            "literary_devices": SchemaProperty(
                "array", "Notable literary devices used", items={"type": "string"}
            ),
        },
    )
...
def _create_quote_details_schema() -> Schema:
    ...
    return Schema(
        ...
        "Detailed information about a quote's origin and type",
        {
            ...
            "date": SchemaProperty(
                "string", "When the quote was originally made (approximate if unknown)"
            ),
...
def generate_quote_metadata(
    quote_text: str, model: str = DEFAULT_MODEL
...
def enrich_quote_with_llm(
    db_session: Session, quote_id: int, model: str = DEFAULT_MODEL, min_confidence: float = 0.75
) -> Quote:
    """
    ...
    try:
        ...
        if confidence < min_confidence:
            ...
            raise LLMUncertainError(
                confidence=confidence, message=f"LLM confidence too low for quote {quote_id}"
            )
        ...
        db_session.commit()
        logger.info(
            f"Successfully enriched quote {quote_id} with LLM data (confidence: {confidence:.1%})"
        )
        return quote
...
def search_quotes(
    db_session: Session, search_term: str, quote_type: Optional[str] = None
) -> List[Quote]:
    """
...
def update_quote(db_session: Session, quote_id: int, quote_data: Dict) -> Optional[Quote]:
    """
