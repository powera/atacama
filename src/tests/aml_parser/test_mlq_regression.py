...
class ColoredMLQTest(unittest.TestCase):
    ...
    def test_multiple_mlqs(self):
        ...
        self.assertEqual(
            ast.children[1].type, NodeType.NEWLINE, "Nodes should be separated by newline."
        )
        ...
        self.assertFalse(
            hasattr(second_mlq, "color"), "Second MLQ should NOT have a color attribute"
        )

        # Test HTML generation to ensure correct wrapping
        html = generate_html(ast)
        self.assertEqual(
            html.count('<div class="mlq color-blue">'), 1, "Should have exactly one blue colorblock"
        )

    def test_multiple_color_names(self):
        """Test that all valid color names work with colored MLQs."""
        color_names = ["xantham", "red", "orange", "blue", "green", "gray", "hazel"]
        ...
        for color in color_names:
            ...
            html = generate_html(ast)
            self.assertIn(
                f'<div class="mlq color-{color}">', html, f"HTML should include {color} block"
            )
