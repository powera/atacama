...
class TemplateTests(unittest.TestCase):
    ...
    def login(self, user_data: Dict[str, Any]) -> None:
        ...
        with self.client.session_transaction() as session:
            session["user"] = {"email": user_data["email"], "name": user_data["name"]}
    ...
    def test_stream_template(self):
        ...
        with self.app.app_context():
            ...
            response = self.client.get(
                url_for("content.message_stream"), headers={"Accept": "text/html"}
            )
            self.assertEqual(response.status_code, 200)
    ...
    def test_login_template(self):
        ...
        with self.app.app_context():
            response = self.client.get(url_for("auth.login"), headers={"Accept": "text/html"})
            self.assertEqual(response.status_code, 200)
