...
class TestSampleCompilation(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.samples_dir = os.path.join(
            os.path.dirname(__file__), "..", "..", "react_compiler", "samples"
        )
    ...
    def test_hook_detection_and_inlining(self):
        ...
        for hook in expected_hooks:
            self.assertIn(
                hook, hooks_needed, f"Hook {hook} should be detected in Basketball sample"
            )

        logger.info(f"Detected hooks: {hooks_needed}")

        # Test hook inlining
        prepared_code = self.widget_builder._prepare_widget_code_with_hooks(
            widget_code, hooks_needed
        )
...
"""
        ...
        success, compiled_code, error_message = self.widget_builder.build_widget(
            widget_code=widget_code, widget_name=widget_name, dependencies=dependencies
        )
