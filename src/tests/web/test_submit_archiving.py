...
class TestSubmitArchiving(unittest.TestCase):
    ...
    def test_archiving_domain_selection(self):
        ...
        for domain_key, domain_config in domains.items():
            if (domain_config.auto_archive_enabled and 
                domain_config.channel_allowed("religion")):
        ...
        for domain_key, domain_config in domains.items():
            if (domain_config.auto_archive_enabled and 
                domain_config.channel_allowed("wikipedia")):
        ...
        for domain_key, domain_config in domains.items():
            if (domain_config.auto_archive_enabled and 
                domain_config.channel_allowed("misc")):
    ...
    def test_submit_archiving_integration(self, mock_get_domain_manager, mock_get_archive_service):
        ...
        mock_archive_service.archive_urls_from_content.return_value = 2  # 2 URLs archived from content
