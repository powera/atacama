...
def navigable(
    ...
    def decorator(route_func: Callable) -> Callable:
        ...
        if needs_auth is None:
            ...
            func = route_func
            while hasattr(func, "__wrapped__"):
                if hasattr(func, "__auth_required__") or func.__name__ == "decorated_function":
                    needs_auth = True
                    break
                func = func.__wrapped__
...
def get_navigation_items(user: Optional[Any] = None) -> Dict[str, List[Dict[str, Any]]]:
    ...
    for blueprint, routes in _navigable_routes.items():
        for route in routes:
            ...
            if route["requires_admin"] and (
                user is None
                or not hasattr(user, "admin_channel_access")
                or not user.admin_channel_access
            ):
                continue
