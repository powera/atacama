        ...
        with db.session() as db_session:
            ...
            if token_obj:
                ...
                if token_age > timedelta(days=TOKEN_EXPIRATION_DAYS):
                    ...
                    logger.warning(
                        f"Auth token expired for user {token_obj.user.email}. Token age: {token_age.days} days"
                    )
                ...
                logger.warning(
                    f"Auth token provided but not found in database: {auth_token[:8]}... (truncated)"
                )
                g.user = None
                return
...
def require_admin(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if not user_config_manager.is_admin(g.user.email):
            return (
                render_template("error.html", error="Admin access required", title="Forbidden"),
                403,
            )
