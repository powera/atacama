...
def get_database_stats():
    ...
    try:
        with db.session() as db_session:
            ...
            return {"status": "Connected", "type": db._engine.dialect.name, "table_stats": stats}
    except Exception as e:
        logger.error(f"Database connection error: {str(e)}")
        return {"status": "Error", "error": str(e), "table_stats": {}}
...
def get_widget_stats():
    ...
    try:
        with db.session() as db_session:
            ...
            published_count = (
                db_session.query(ReactWidget).filter(ReactWidget.published == True).count()
            )

            # Get draft (unpublished) widgets
            draft_count = (
                db_session.query(ReactWidget).filter(ReactWidget.published == False).count()
            )
...
def debug_login() -> ResponseReturnValue:
    ...
    session["user"] = {"name": name, "email": email, "provider": "debug"}
