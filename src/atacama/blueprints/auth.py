...
def callback() -> ResponseReturnValue:
    ...
    try:
        ...
        with db.session() as db_session:
            ...
            if mobile_mode and mobile_redirect:
                ...
                new_token = UserToken(user_id=db_user.id, token=auth_token, device_info=device_info)
                db_session.add(new_token)
                db_session.commit()

                logger.info(
                    f"Generated auth token for user {db_user.email} (mobile OAuth, device: {device_info})"
                )
    ...
    if popup_mode:
        ...
        return render_template("login_success_popup.html", user=session["user"])
    else:
