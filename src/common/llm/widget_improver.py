...
Ensure the widget provides an excellent mobile user experience.""",
    ...
    def improve_widget(
        ...
        try:
            ...
            logger.info(
                f"Improving widget code for '{widget_title}' with prompt: {full_prompt[:100]}..."
            )
            ...
            include_data_file = (
                "data_file" in target_files or "create_data_file" in target_files or bool(data_file)
            )

            # Use dynamic schema based on target files
            improvement_schema = create_widget_improvement_schema(
                include_data_file=include_data_file
            )
            ...
            logger.info(
                f"Finished improving widget code for '{widget_title}'. Target files: {target_files}"
            )
            return {
                "success": True,
    ...
    def _extract_code_and_data_from_response(
        ...
        if not data_content:
            ...
            if data_section_match:
                ...
                if re.search(r"\{.*\}", response, re.DOTALL) and not any(
                    ext in response for ext in [".jsx", ".js", ".tsx", ".ts", ".css", ".md"]
                ):
                    try:
                        import json
