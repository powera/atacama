...
class RequestLogger:
    ...
    def init_app(self, app):
        ...
        info_handler = RotatingFileHandler(
            os.path.join(self.log_dir, "requests.log"), maxBytes=10000000, backupCount=10  # 10MB
        )
        ...
        def after_request(response):
            ...
            if request.args:
                sanitized_params = {
                    k: v for k, v in request.args.items() if k.lower() not in self.SENSITIVE_PARAMS
                }
                if sanitized_params:
            ...
            if request.method in ["POST", "PUT"] and request.is_json:
                ...
                if body and isinstance(body, dict):
                    sanitized_body = {
                        k: v for k, v in body.items() if k.lower() not in self.SENSITIVE_PARAMS
                    }
                    if sanitized_body:
                        ...
                        if len(body_json.encode("utf-8")) > 480:
                            ...
                            truncated_body = body_json.encode("utf-8")[:480].decode(
                                "utf-8", errors="ignore"
                            )
