...
class ChannelManager:
        ...
        try:
            ...
            for name, settings in channel_configs.items():
                ...
                if group not in self.valid_groups:
                    raise ValueError(
                        f"Invalid group '{group}' for channel '{name}'. Valid groups are: {', '.join(self.valid_groups)}"
                    )
    ...
    def _validate_config(self) -> None:
        ...
        for channel in self.default_preferences:
            if channel not in self.channels:
                raise ValueError(
                    f"Default preference channel '{channel}' not found in channel list"
                )
    ...
    def get_public_channels(self) -> List[str]:
        ...
        return [name for name, config in self.channels.items() if config.is_public]
    ...
    def check_system_access(
        self, channel_name: str, email: Optional[str] = None, has_admin_access: bool = False
    ) -> bool:
        """
