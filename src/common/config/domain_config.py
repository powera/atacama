...
class DomainManager:
    ...
    def _load_config(self) -> None:
        ...
        try:
                ...
                if settings.get("domains"):
                    for hostname in settings.get("domains", []):
                        if hostname in self.host_to_domain:
                            ...
                            logger.error(
                                f"Duplicate hostname '{hostname}' found in domains '{domain_key}' and '{existing_domain}'"
                            )
                            raise ValueError(
                                f"Duplicate hostname '{hostname}' in multiple domain configurations"
                            )
            ...
            logger.info(
                f"Domain configuration loaded successfully: {len(self.domains)} domains, {len(self.themes)} themes"
            )
            for domain_key, config in self.domains.items():
                channel_info = (
                    "all channels"
                    if config.allows_all_channels
                    else f"{len(config.channels)} channels"
                )
                logger.info(
                    f"  Domain '{domain_key}': {config.name}, theme: {config.theme}, {channel_info}"
                )
    ...
    def _validate_config(self) -> None:
        ...
        for domain_key, domain_config in self.domains.items():
            if domain_config.theme not in self.themes:
                raise ValueError(
                    f"Theme '{domain_config.theme}' referenced by domain '{domain_key}' not found"
                )
