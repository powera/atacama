...
from atacama.decorators import (
    navigable,
    navigable_per_channel,
    optional_auth,
    require_admin,
    require_auth,
)
...
@optional_auth
@navigable_per_channel(name="Message Stream", description="Stream for this channel", order=100)
def message_stream(
    older_than_id: Optional[int] = None,
    channel: Optional[str] = None,
    tsdate: Optional[str] = None,
    tstime: Optional[str] = None,
) -> ResponseReturnValue:
    """
    ...
    with db.session() as db_session:
        try:
            ...
            query = (
                ...
                .options(joinedload(Email.parent), joinedload(Email.children))
...
def all_messages(tsdate: Optional[str] = None, tstime: Optional[str] = None) -> ResponseReturnValue:
    ...
    with db.session() as db_session:
        ...
        query = (
            ...
            .filter(Message.channel.in_(allowed_channels))
        )
        ...
        for message in messages:
            ...
            if message.message_type.value == "email":
                ...
                if article and article.published:
                    ...
                    title = article.title
                    preview = (
                        article.processed_content[:200] + "..."
                        if article.processed_content and len(article.processed_content) > 200
                        else article.processed_content
                    )
                ...
                if quote:
                    ...
                    title = (
                        f"{quote.quote_type.title()}: {quote.text[:50]}..."
                        if len(quote.text) > 50
                        else f"{quote.quote_type.title()}: {quote.text}"
                    )
                    preview = f"By {quote.original_author or 'Unknown'}" + (
                        f" - {quote.commentary[:100]}..." if quote.commentary else ""
                    )
...
@optional_auth
@navigable_per_channel(name="Message List", description="List messages for a channel", order=100)
