...
def edit_widget(slug: str) -> ResponseReturnValue:
    ...
    with db.session() as session:
        ...
        if not (
            g.user.id == widget.author_id
            or (g.user.admin_channel_access and widget.channel in g.user.admin_channel_access)
        ):
        ...
        if request.method == "POST":
            ...
            existing_version = (
                session.query(WidgetVersion)
                .filter_by(widget_id=widget.id, code_hash=code_hash)
                .first()
            )
            ...
            if not existing_version:
                ...
                logger.info(
                    f"Created version {version_number} for widget {slug}, build success: {build_success}"
                )
        ...
        return render_template(
            "widgets/edit_widget.html", widget=widget, channel_manager=get_channel_manager()
        )
    ...
    if request.method == "POST":
        ...
        with db.session() as session:
            ...
            if code.strip():
                ...
                logger.info(
                    f"Created initial version for widget {slug}, build success: {build_success}"
                )
...
def build_widget(slug: str) -> ResponseReturnValue:
    ...
    if success:
        ...
        return jsonify({"status": "error", "message": f"Build failed!  Check server logs."}), 400
...
def publish_widget(slug: str) -> ResponseReturnValue:
    ...
    with db.session() as session:
        ...
        return jsonify({"status": "success", "message": "Widget published!"}), 200
...
def improve_widget(slug: str) -> ResponseReturnValue:
    ...
    with db.session() as session:
        ...
        if request.method == "GET":
            ...
            return render_template("widgets/improve.html", widget=widget)
        ...
        if not prompt.strip():
            return jsonify({"success": False, "error": "Improvement prompt is required"}), 400
        ...
        if base_version == "current":
            ...
            if not version or version.widget_id != widget.id:
                return jsonify({"success": False, "error": "Invalid version selected"}), 400
        ...
        improvement_jobs[job_id] = {
            ...
            "widget_info": {"slug": slug, "title": widget_title},
        }
        ...
        def improve_in_background():
            try:
                ...
                logger.error(
                    f"Error in background improvement for widget {slug}, job {job_id}: {str(e)}"
                )
        ...
        return jsonify(
            {"success": True, "job_id": job_id, "message": "Improvement started in background"}
        )
...
def improve_status(slug: str, job_id: str) -> ResponseReturnValue:
    ...
    with db.session() as session:
        ...
        if job["status"] in ["completed", "error"] and current_time - job["started_at"] > 86400:
            ...
            return jsonify({"success": False, "error": "Job has expired"}), 404
...
def save_version(slug: str) -> ResponseReturnValue:
    ...
    with db.session() as session:
        ...
        existing_version = (
            session.query(WidgetVersion).filter_by(widget_id=widget.id, code_hash=code_hash).first()
        )
...
def get_version_code(slug: str) -> ResponseReturnValue:
    ...
    with db.session() as session:
        ...
        if version == "current":
            ...
            version_obj = (
                session.query(WidgetVersion).filter_by(id=version, widget_id=widget.id).first()
            )
            if not version_obj:
        ...
        return jsonify({"success": True, "code": code, "data_file": data_file})
    ...
    if request.method == "POST":
        ...
        def initiate_in_background():
            try:
                ...
                dual_file = (
                    widget_schema == DUAL_FILE_WIDGET_SCHEMA
                    if isinstance(widget_schema, Schema)
                    else widget_schema == "dual_file"
                )
                ...
                if not result["success"]:
                    ...
                    improvement_jobs[job_id][
                        "error"
                    ] = f"Failed to generate widget: {result['error']}"
                ...
                with db.session() as session:
                    try:
                        ...
                        build_success = initial_version.build()
                        logger.info(
                            f"Created AI-generated widget {slug}, build success: {build_success}"
                        )
                        ...
                        improvement_jobs[job_id]["finished_at"] = time.time()
                        logger.info(
                            f"Completed background widget initiation for slug {slug}, job {job_id}"
                        )

                    except Exception as e:
                        logger.error(
                            f"Error creating widget in database for job {job_id}: {str(e)}"
                        )
                        improvement_jobs[job_id]["status"] = "error"
...
def list_jobs() -> ResponseReturnValue:
    ...
    for job_id, job_data in improvement_jobs.items():
        if (
            job_data["status"] in ["completed", "error"]
            and current_time - job_data["started_at"] > 86400
        ):
    ...
    return render_template(
        "admin/jobs.html", running_jobs=running_jobs, completed_jobs=completed_jobs
    )
