...
def serialize_message(message: Email) -> Dict[str, Any]:
    ...
    return {
        ...
        "author": (
            {"id": message.author.id, "email": message.author.email, "name": message.author.name}
            if message.author
            else None
        ),
        "quotes": [
        ...
        "chinese_annotations": (
            json.loads(message.chinese_annotations) if message.chinese_annotations else None
        ),
...
def main() -> None:
    ...
    parser.add_argument(
        "--output", "-o", default="messages_export.json", help="Output JSON file path"
    )
    parser.add_argument(
