        ...
        try:
            ...
            if message.children:
                if cascade:
                    ...
                    logger.info(
                        f"Cascading delete for message {message_id} will remove children: {child_ids}"
                    )
                else:
                    ...
                    logger.info(
                        f"Reparenting children of message {message_id} to parent {message.parent_id if message.parent else None}"
                    )
...
def reprocess_message(message_id: int):
    ...
    with db.session() as session:
        try:
            message = (
                session.query(Email)
                .options(joinedload(Email.quotes))
            ...
            for quote in existing_quotes:
                ...
                if (
                    not session.query(email_quotes)
                    .filter(
                        email_quotes.c.quote_id == quote.id, email_quotes.c.email_id != message_id
                    )
                    .first()
                ):
            ...
            message.preview_content = aml_parser.process_message(
                message.content, message=message, db_session=session, truncated=True
            )
    ...
    with db.session() as session:
        try:
            ...
            if not channel_config:
                ...
                return (
                    False,
                    f"Invalid channel '{channel_name}'. Valid channels are: {valid_channels}",
                )
...
def list_uncategorized_messages() -> list:
    ...
    with db.session() as session:
        try:
            messages = session.query(Email.id, Email.subject).filter(Email.channel == None).all()
            return messages
